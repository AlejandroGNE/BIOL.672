...
# didn't work, geom_errorbar creates subgroups within my categories and plots error bars for each subgroup
...
box.plot.agevsfuelcategories <- ggplot(Retirement_dataset, mapping = aes(x = Fuel_categories, y = Age)) + geom_boxplot() + labs(x = 'Fuel categories', y = 'Physical life [years]') + theme(axis.text.x=element_text(angle=15), panel.background = element_rect(fill = 'grey50'))
...
# Class 2020-09-25 - PCA, factor analysis, k-means ----
# PCAdataframe <- data.frame(Age, Capacity)
# myPCA <- prcomp(PCAdataframe, scale = TRUE)
# 
